...
function init(server) {
    ...
    wss.on("connection", (socket) => {
        ...
        socket.on("close", (code, reason) =>
            onClose(wss, socket, code, reason)
        );
    });
}
...
function clearClient(wss, socket, reason) {
    ...
    Object.keys(channels).forEach((cname) => {
        Object.keys(channels[cname]).forEach(async (uid) => {
            if (channels[cname][uid] === socket) {
                if (reason === "S" || hostList[cname] === uid) {
                    delete hostList[cname];
                    delete channels[cname][uid];
                    await clearAllClients(Object.values(channels[cname]));
...
function clearAllClients(arr) {
    console.log("clearAllClients invoked");
    arr.forEach((socket) => {
        socket.send(
            JSON.stringify({
                type: "quit",
                body: {
                    reason: "streamer_quit",
                },
            })
        );

        socket.close(1000, "FQ"); // Forced Quit
