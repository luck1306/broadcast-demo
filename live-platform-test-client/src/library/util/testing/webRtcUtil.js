...
const onAnswer = (props) => {
  ...
  if (localStream.getVideoTracks().length > 0)
    console.log(
      `[Local] Using video device: ${localStream.getVideoTracks()[0].label}`
    );
  if (localStream.getAudioTracks().length > 0)
    console.log(
      `[Local] Using audio device: ${localStream.getAudioTracks()[0].label}`
    );
...
const addCandidate = (candidate) => {
  ...
  if (localPeerConnection?.remoteDescription === null)
